...
def mag_thresh(img):
    ...
    binary_output = cv2.dilate(binary_output, np.ones((5,5),np.uint8), 2)
...
def find_window_centroids(image, window_width, window_height, margin):
    ...
    frac = 0.25 # Lower fraction of image for estimating locaitono of lanes
    ...
    for l in range(int(h/window_height)): # Loop over all the levels (~9)
        ...
        if len(l_cnts) < 2:
            ...
            l_max = int(min(l_center + offset + margin, w))
        else:
            l_proj = l_cnts[-1,0] + (l_cnts[-1,0]-l_cnts[-2,0]) \
                   * ((l - l_lvls[-1])/(l_lvls[-1]-l_lvls[-2]))
            if l_proj < 0: l_proj = 0
            if l_proj > w: l_proj = w
            l_min = int(max(l_proj - offset - margin, 0))
            l_max = int(min(l_proj + offset + margin, w))
        ...
        if len(r_cnts) < 2:
            ...
            r_max = int(min(r_center + offset + margin, w))
        else:
            r_proj = r_cnts[-1,0] + (r_cnts[-1,0]-r_cnts[-2,0]) \
                   * ((l - r_lvls[-1])/(r_lvls[-1]-r_lvls[-2]))
            if r_proj < 0: r_proj = 0
            if r_proj > w: r_proj = w
            r_min = int(max(r_proj - offset - margin, 0))
            r_max = int(min(r_proj + offset + margin, w))
...
def polyfit_and_compute_radcurv(sws_flag, lcpts, rcpts, lline, rline, margin):
    ...
    if len(lline.current_fit) == 0:
        ...
        if debug_log:
            with open("log.txt", "a") as myfile:
                myfile.write('\n --- chk1_left ---')
                myfile.write('\n x_tdiff {0:.4f} (>{1})'.format(lx_tdiff,5.0*margin))
    ...
    if len(rline.current_fit) == 0:
        ...
        if debug_log:
            with open("log.txt", "a") as myfile:
                myfile.write('\n --- chk1_right ---')
                myfile.write('\n x_tdiff {0:.4f} (>{1})'.format(rx_tdiff,5.0*margin))
    ...
    y_test = np.linspace(0,700,8) ; lwmin = 5*0.3048 ; lwmax = 30*0.3048
...
make_gif = False
if make_gif:
    vfile = mpy.VideoFileClip('project_output.mp4')
    vclip = vfile.subclip(t_start=19.96, t_end=30.08)
    vclip = vclip.resize(0.5).crop(x1=50,x2=50,y1=50,y2=50)
    vclip.write_gif('project_output_2.gif', fps=10, program='ffmpeg', opt='nq')
